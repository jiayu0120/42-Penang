...
int	valid_placement(char grid[7][7], int row, int column, int value)
{
	int	i;
	int	j;
...
int	check_partial(char grid[7][7], int row, int col, char *clues)
{
	int	left;
	int	top;
	...
	if (left > clues[row + 14] - '0')
		return (0);
	if (left + (6 - col) < clues[row + 14] - '0')
		return (0);
	if (top > clues[col] - '0')
		return (0);
	if (top + (6 - row) < clues[col] - '0')
		return (0);
	return (1);
}

int	check_finished(char grid[7][7], int row, int col, char *clues)
{
	if (col == 6)
	{
		if (visi_row_left(grid, row) != clues[row + 14] - '0')
			return (0);
		if (visi_row_right(grid, row) != clues[row + 21] - '0')
			return (0);
	}
	if (row == 6)
	{
		if (visi_col_top(grid, col) != clues[col] - '0')
			return (0);
		if (visi_col_bottom(grid, col) != clues[col + 7] - '0')
			return (0);
	}
	return (1);
}

int	check_position(char grid[7][7], int row, int col, char *clues)
{
...
int	solve_square(char grid[7][7], int row, int column, char *clues)
{
	int	num;
